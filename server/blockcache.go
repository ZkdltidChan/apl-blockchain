...
func (bc BlockCache) PopulateWithBlocks(client cv3.Client, start int, end int) error {
	...
	resp, err := client.Get(
		...
		fmt.Sprintf("%s%s", newPrefix, intToBinary(int64(start*st.DEFAULT_BLOCK_SIZE))), // Start key

		// cv3.WithRange(fmt.Sprintf("%s%s", PREFIX, intToBinary(int64(end*st.DEFAULT_BLOCK_SIZE)))), // End key
		cv3.WithRange(fmt.Sprintf("%s%s", newPrefix, intToBinary(int64(end*st.DEFAULT_BLOCK_SIZE)))), // End key
